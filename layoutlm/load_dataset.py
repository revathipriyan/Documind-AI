...
class InvoiceExtraction(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        # Choose the directory that actually contains class_list.txt / train.txt if possible.
        candidate = os.path.join(data_path, 'layoutlmv3')
        if os.path.exists(os.path.join(data_path, 'class_list.txt')) and os.path.exists(os.path.join(data_path, 'train.txt')):
            dest = data_path
        ...
        print(f"Using dataset directory: {os.path.abspath(dest)}")
